 ...
 */
 ...
 void
 QueueDisc::Drop(Ptr<const QueueDiscItem> item, const char* reason)
 {
     NS_LOG_FUNCTION(this << item << reason);
 ...
 QueueDisc::Dequeue()
 {
     ...
     {
         m_requeued = nullptr;
         NS_LOG_INFO("Dequeueing a requeued packet");
         if (m_peeked)
         {
     ...
     NS_ASSERT_MSG(
         m_nPackets == m_stats.nTotalEnqueuedPackets - m_stats.nTotalDequeuedPackets,
         "m_nPacket = " << m_nPackets << " nTotalEnqueuedPackets = " << m_stats.nTotalEnqueuedPackets
                        << " - nTotalDequeuedPackets = " << m_stats.nTotalDequeuedPackets);
 ...
 QueueDisc::Transmit(Ptr<QueueDiscItem> item)
 {
     ...
     {
         NS_LOG_INFO("Requeueing packet. Failed to transmit.");
         Requeue(item);
         return false;
     }
