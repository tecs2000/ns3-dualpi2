...
LteEnbMac::DoReceiveLteControlMessage(Ptr<LteControlMessage> msg)
{
    ...
    if (msg->GetMessageType() == LteControlMessage::DL_CQI)
    {
        ...
        /**
         * Pipe the info up to the RLC Layer.
         *
         * After observation, was noticed that the vector
         * only contains data in its first position.
         */
        auto rnti = dlcqi->GetDlCqi().m_rnti;
        auto outerIt = m_rlcAttached.find(rnti); // returns every RLC instance associated to this rnti
        
        NS_ASSERT(outerIt != m_rlcAttached.end());
        NS_LOG_INFO(this << " rnti " << rnti << " has RLC instances attached");

        std::vector<uint8_t> cqis = dlcqi->GetDlCqi().m_wbCqi;
        if(!cqis.empty()){
            if (outerIt != m_rlcAttached.end())
            {
                const std::map<uint8_t, LteMacSapUser*>& innerMap = outerIt->second;
                NS_LOG_INFO(this << " Number of channels: " << innerMap.size());

                for (const auto& innerPair : innerMap)
                {
                    uint8_t lcid = innerPair.first;
                    LteMacSapUser* rlcInstance = innerPair.second;
                    rlcInstance->SetCqi(rnti, lcid, cqis[0]);
                }
            }
            else
            {
                NS_LOG_INFO(this << " RNTI not found in the map.");
            }
