...
class LteRlcSapUser
{
  public:
    ...
    /**
     * Called by the RLC entity to notify the PDCP entity of the reception of a new CQI data
     *
     * \param p the CQI data
     */
    virtual void ReceiveRlcCQI(uint8_t cqi) = 0;
};
...
class LteRlcSpecificLteRlcSapUser : public LteRlcSapUser
{
  public:
    ...
    // Interface implemented from LteRlcSapUser
    void ReceiveRlcCQI(uint8_t cqi) override;
...
template <class C>
void
LteRlcSpecificLteRlcSapUser<C>::ReceiveRlcCQI(uint8_t cqi)
{
    m_pdcp->DoReceiveRlcCQI(cqi);
}
