...
class NrSimpleUeCcmMacSapUser : public NrMacSapUser
{
  public:
    ...
    void SetCqi(uint16_t rnti, uint8_t lcid, uint8_t cqi) override;
...
void NrSimpleUeCcmMacSapUser::SetCqi(uint16_t rnti, uint8_t lcid, uint8_t cqi)
{
    m_mac->DoSetCqi(rnti, lcid, cqi);
}
...
void
NrSimpleUeComponentCarrierManager::DoSetCqi(uint16_t rnti, uint8_t lcid, uint8_t cqi)
{
    NS_LOG_FUNCTION(this);
    auto lcidIt = m_lcAttached.find(lcid);
    NS_ABORT_MSG_IF(lcidIt == m_lcAttached.end(), "could not find LCID" << (uint16_t)lcid);
    if (lcidIt != m_lcAttached.end())
    {
        (*lcidIt).second->SetCqi(rnti, lcid, cqi);
    }
}
