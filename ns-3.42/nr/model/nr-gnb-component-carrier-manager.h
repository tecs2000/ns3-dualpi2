...
void
NrGnbComponentCarrierManager::DoSetCqi(uint16_t rnti, uint8_t lcid, uint8_t cqi)
{
    NS_LOG_FUNCTION(this);
    auto lcidIt = m_ueInfo.at(rnti).m_ueAttached.find(lcid);
    if (lcidIt != m_ueInfo.at(rnti).m_ueAttached.end())
    {
