...
NrGnbMac::DoReceiveControlMessage(Ptr<NrControlMessage> msg)
{
    ...
    case (NrControlMessage::DL_CQI): {
        ...
        uint8_t cqiValue = cqiElement.m_wbCqi;

        if (outerIt != m_rlcAttached.end())
        {
            const std::unordered_map<uint8_t, NrMacSapUser*>& innerMap = outerIt->second;
            NS_LOG_INFO(this << " Number of channels: " << innerMap.size());
            for (const auto& innerPair : innerMap)
            {
                uint8_t lcid = innerPair.first;
                NrMacSapUser* rlcInstance = innerPair.second;
                rlcInstance->SetCqi(rnti, lcid, cqiValue);
            }
        }
        else
        {
