...
main(int argc, char* argv[])
{
    ...
    // LogComponentEnable("DualQCoupledPiSquareQueueDisc", LOG_LEVEL_ALL);
    ...
    // Configure LL video stream client
    VideoStreamClientHelper videoClientll(internetIpIfaces.GetAddress(1));
    videoClientll.SetAttribute("TCPType", StringValue("TcpDctcp"));
    ApplicationContainer llclientApp = videoClientll.Install(ueLowLatContainer);

    // Set DCTCP as the LL client congestion control
    Ptr<TcpL4Protocol> protoll;
    protoll = ueLowLatContainer.Get(0)->GetObject<TcpL4Protocol>();
    protoll->SetAttribute("SocketType", TypeIdValue(TcpDctcp::GetTypeId()));
    llclientApp.Start(appStartTime);
    llclientApp.Stop(simTime);

    // Configure classic video stream client
    VideoStreamClientHelper videoClientclassic(internetIpIfaces.GetAddress(1));
    ApplicationContainer classicclientApp = videoClientclassic.Install(ueLowLatContainer);

    // Set Cubic as the classic client congestion control
    Ptr<TcpL4Protocol> protoclassic;
    protoclassic = ueVoiceContainer.Get(0)->GetObject<TcpL4Protocol>();
    protoclassic->SetAttribute("SocketType", TypeIdValue(TcpCubic::GetTypeId()));
    classicclientApp.Start(appStartTime);
    classicclientApp.Stop(simTime);

    // Creates one video stream app installed in the remote host
    VideoStreamServerHelper videoServer(5000);
    videoServer.SetAttribute("MaxPacketSize", UintegerValue(1400));
    videoServer.SetAttribute("FrameFile", StringValue("scratch/files/bigFrameList.txt"));
    ApplicationContainer serverAppclassic = videoServer.Install(remoteHost);
    serverAppclassic.Start(appStartTime);
    serverAppclassic.Stop(simTime);
